...
`web/` 資料夾即完整網頁遊戲，採**即時放置玩法**：
- 左側掛機日誌**逐條即時冒出**，狀態列隨戰鬥即時跳動（HP/SP/金錢/效率）
- 右側 `config.txt` **隨時編輯 → 按「套用設定」即時生效，角色狀態(等級/金錢)保留**
  → 所以可以**先在低等圖練強，再中途切去高等地圖**
- **開始／暫停／重置(LV1)** 按鈕；速度可調（快/中/慢）
